...
type State = {
  ...
  /** Tracks which driver/load to simulate — persists past toast dismissal. */
  playbackAssign: { driverId: string; loadId: string } | null;
  ...
  switch (action.type) {
    ...
    case "assign": {
      ...
      return {
        ...
        playbackAssign: { loadId: action.loadId, driverId: action.driverId },
...
function useDispatchValue() {
  ...
  const driversSimulated: Driver[] = useMemo(() => {
    const pa = state.playbackAssign;
    return DRIVERS.map((d) => {
      if (pa && d.id === pa.driverId) {
        const assignedLoad = LOADS.find((l) => l.id === pa.loadId) ?? null;
        return driverForSimulation(d, assignedLoad, state.simulatedHoursOffset);
      }
      return driverForSimulation(d, selectedLoad, pa ? 0 : state.simulatedHoursOffset);
    });
  }, [selectedLoad, state.simulatedHoursOffset, state.playbackAssign]);
